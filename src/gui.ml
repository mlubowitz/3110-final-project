...
let notation s loc_x loc_y = 
  notation_num s loc_x (loc_y + (s/2) + 20) ["1"; "2"; "3"; "4"; "5"; "6"; "7"; "8"];
  notation_words s (loc_x + (s/2)  + 20) (loc_y  + (s * 8) + (s/2) - 5) ["A";"B"; "C"; "D"; "E"; "F"; "G"; "H"];;


let rec pawns count s loc_x loc_y bl = 
  match count with 
  | 0 -> moveto loc_x loc_y;
         if bl then set_color black else set_color white;
         draw_string "PAWN" ;
  | _ -> moveto loc_x loc_y;
         if bl then set_color black else set_color white;
         draw_string "PAWN";
...
let init_setup init_x init_y s =
  ...
  pieces s (init_x + s/3) (init_y + (s/2) - 10) false 
  ["ROOK"; "KNIGHT"; "BISHOP"; "QUEEN"; "KING"; "BISHOP"; "KNIGHT"; "ROOK"] ;
  pawns 7 s (init_x  + s/3) ((init_y * 8) - 10) true;
  pieces s (init_x  + s/3) ((init_y * 9) - 10) true 
